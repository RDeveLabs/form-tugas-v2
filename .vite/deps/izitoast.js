...
//#region node_modules/izitoast/dist/js/iziToast.js
var require_iziToast = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(root, factory) {
		if (typeof define === "function" && define.amd) define([], factory(root));
		else if (typeof exports === "object") module.exports = factory(root);
		else root.iziToast = factory(root);
	})(typeof global !== "undefined" ? global : window || exports.window || exports.global, function(root) {
		"use strict";
		var $iziToast = {}, PLUGIN_NAME = "iziToast";
		document.querySelector("body");
		var ISMOBILE = /Mobi/.test(navigator.userAgent) ? true : false, ISCHROME = /Chrome/.test(navigator.userAgent) && /Google Inc/.test(navigator.vendor), ISFIREFOX = typeof InstallTrigger !== "undefined", ACCEPTSTOUCH = "ontouchstart" in document.documentElement, POSITIONS = [
			"bottomRight",
			"bottomLeft",
			"bottomCenter",
			"topRight",
		...
		*/
		var forEach = function(collection, callback, scope) {
			...
			} else if (collection) for (var i = 0, len = collection.length; i < len; i++) callback.call(scope, collection[i], i, collection);
		};
		/**
		...
		*/
		var generateId = function(params) {
			return btoa(encodeURIComponent(params)).replace(/=/g, "");
		};
		/**
		* Check if is a color
		* @private
			...
			return {
				...
				startMoving: function(toast, instance, settings, e) {
					...
					if (ACCEPTSTOUCH) document.ontouchmove = function(e) {
						e.preventDefault();
						e = e || window.event;
						var finalX = e.touches[0].clientX - offsetX;
			...
			return {
				...
				pause: function() {
					if (typeof settings.time.START !== "undefined" && !$toast.classList.contains(PLUGIN_NAME + "-paused") && !$toast.classList.contains(PLUGIN_NAME + "-reseted")) {
						...
						that.setSetting(ref, "time", settings.time);
						if ($elem !== null) {
							var propertyWidth = window.getComputedStyle($elem).getPropertyValue("width");
		...
		*/
		$iziToast.show = function(options) {
			...
			(function() {
				...
				if (settings.class) forEach(settings.class.split(" "), function(value, index) {
				...
				if (settings.maxWidth) if (!isNaN(settings.maxWidth)) $DOM.toast.style.maxWidth = settings.maxWidth + "px";
				...
				if (settings.color) if (isColor(settings.color)) $DOM.toast.style.background = settings.color;
			...
			(function() {
				if (settings.title.length > 0) {
					...
					if (settings.titleSize) if (!isNaN(settings.titleSize)) $DOM.strong.style.fontSize = settings.titleSize + "px";
					else $DOM.strong.style.fontSize = settings.titleSize;
					if (settings.titleLineHeight) if (!isNaN(settings.titleSize)) $DOM.strong.style.lineHeight = settings.titleLineHeight + "px";
				...
				if (settings.message.length > 0) {
					...
					if (settings.messageLineHeight) if (!isNaN(settings.titleSize)) $DOM.p.style.lineHeight = settings.messageLineHeight + "px";
			...
			if (settings.inputs.length > 0 || settings.buttons.length > 0) {
				...
				if (settings.inputs.length > 0 && settings.buttons.length > 0) if (settings.rtl) $DOM.inputs.style.marginLeft = "8px";
			...
			if (settings.drag) if (ACCEPTSTOUCH) {
...
//#endregion
export default require_iziToast();

//# sourceMappingURL=izitoast.js.map